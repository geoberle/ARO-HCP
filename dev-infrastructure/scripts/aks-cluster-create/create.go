...
func run(ctx context.Context, o *completedOptions, logger logr.Logger) error {
	...
	// without mutating anything. The fleet controller will reconcile the cluster.
	...
	if compute.RequiredTierFailed(failures) {
		return fmt.Errorf("required tier allocation failed: %s", compute.FailureSummary(failures))
	}
	if len(systemPools) == 0 {
		return fmt.Errorf("no system pool could be allocated: %s", compute.FailureSummary(failures))
	}
	...
	// Re-PUT on resume is safe: the bootstrap-only profile does not delete agent pools created out-of-band by a prior run; the ensurePool loop below reconciles them.
...
// Pools that already exist and are not Failed are left as-is. Transient failures
// are handled by the SDK retry policy and the pipeline's resumable re-run.
