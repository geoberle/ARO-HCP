...
// current usage. It returns the desired pools and any allocation
// failures explaining why tiers could not be satisfied.
//
	...
	for _, family := range familyAllocationOrder(tier, zones, skuIndex) {
		...
		case PoolModeRegional:
			...
			pools = append(pools, Pool{
				Role:              tier.Role,
				Name:              poolName(tier.Name, "0", vmSize, tier.OSDiskSizeGB, tier.MaxPods, tier.EnableSwift),
		...
		case PoolModePerZone:
			...
			for _, zone := range poolZones {
				pools = append(pools, Pool{
					Role:              tier.Role,
					Name:              poolName(tier.Name, zone, vmSize, tier.OSDiskSizeGB, tier.MaxPods, tier.EnableSwift),
					Spec:              NewVMSpecFromSKU(meta),
...
// hex prefix of the SHA-256 of the pool's AKS-immutable fields (VMSize,
// OSDiskSizeGB, MaxPods, EnableSwift). Changing any of those fields changes the
...
func poolName(symbolicName string, zone string, vmSize string, osDiskSizeGB int32, maxPods int32, enableSwift bool) string {
	input := fmt.Sprintf("%s|%d|%d|%t", vmSize, osDiskSizeGB, maxPods, enableSwift)
