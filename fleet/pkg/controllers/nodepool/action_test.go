...
func formatTrace(tr trace) string {
	...
	for i, step := range tr.Steps {
		...
		action := fmt.Sprintf("  %3d. %-16s %-7s %-18s %-10s %-14s",
...
func actionDetail(a Action) string {
	switch v := a.(type) {
	case createAction:
		return fmt.Sprintf("min=%d max=%d", v.Pool.InitialMinCount, v.Pool.MaxCount)
	case setScalingBoundsAction:
		return fmt.Sprintf("min=%d max=%d", v.MinCount, v.MaxCount)
	case unfreezeAction:
		return fmt.Sprintf("min=%d max=%d", v.MinCount, v.MaxCount)
	case freezeAction:
...
func TestUndesiredSystemPoolUnblocksUserPools(t *testing.T) {
	...
	tr := simulateAndTrace(t, desired, initial, generousBudgets(desired, initial), 20)
	assertConverged(t, desired, tr.finalState())
}
...
func TestCreateSeedsMinCountFromTier(t *testing.T) {
	...
	action := findNextAction(desired, nil, generousBudgets(desired, nil), compute.NetworkConfig{})
	create, ok := action.(createAction)
...
func TestCreateReclampsMinToFitHeadroom(t *testing.T) {
	...
	action := findNextAction(desired, nil, budgets, compute.NetworkConfig{})
	create, ok := action.(createAction)
	require.True(t, ok, "expected a create action")
...
func TestUnfreezePreservesLiveMin(t *testing.T) {
	desired := []compute.Pool{
		pool("s1", specE32v6, "1", 6, 512),
	}
	cur := poolState("s1", specE32v6, "1", 6, 512, false, 2)
	cur.MinCount = 3
	current := []PoolState{cur}

	action := findNextAction(desired, current, generousBudgets(desired, current), compute.NetworkConfig{})
	requireAction(t, newUnfreezeAction("s1", "Standard_E32ds_v6", "1", "etag-s1", 3, 6), action)
}
...
func TestConfigRollback(t *testing.T) {
	...
	action := findNextAction(desired, current, generousBudgets(desired, current), compute.NetworkConfig{})
	require.NotNil(t, action)
	requireAction(t, newUnfreezeAction("s1abc", "Standard_E32ds_v6", "1", "etag-s1abc", 1, 6), action)
}
...
func TestProductionScenario_MigrationToDesired(t *testing.T) {
	...
	// resolved.AvailableVCPUs from the real quota snapshot: limit - usage.
