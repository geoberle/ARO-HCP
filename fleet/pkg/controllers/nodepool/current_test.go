...
func TestCurrentPoolStates(t *testing.T) {
	tests := []struct {
		...
		{
			name: "taints maxPods swift and resolved SKU are projected",
			skuMetadata: map[string]*skucache.SKUMetadata{
			...
			pools: []armcontainerservice.AgentPool{
				{
					...
					Properties: &armcontainerservice.ManagedClusterAgentPoolProfileProperties{
						...
						Tags: map[string]*string{
							...
							agentpoolspec.SwiftSecondaryNICCountTag: ptr.To("7"),
						},
					},
				},
			},
			expected: []PoolState{
				{
					Pool: compute.Pool{
						...
						Spec:              compute.VMSpec{Size: "Standard_E16ds_v6", Family: "standardEDSv6Family", VCPUs: 16, MemoryGB: 128, SecondaryNICs: 7},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			result := currentPoolStates(test.pools, test.skuMetadata)
