...
config value (`ci`, `development`, `production`). Profiles are
...
| `PoolMode` | `PerZone` (`PoolCount` pools, one per AZ; needs a SKU in every target zone) or `Regional` (one pool with no zones set, AKS places nodes anywhere in the region; the only mode that can use zone-restricted SKUs). `PoolCount` is honored **per SKU**: if one SKU cannot fill `MaxNodes`, the next family in `FamilyPriority` stacks another `PoolCount`-set of the next SKU until the tier is filled — so a tier may emit more than `PoolCount` pools |
...
the tier's **AKS-immutable** fields (`VMSize`, `OSDiskSizeGB`, `MaxPods`,
`EnableSwift`). The controller matches desired against live pools by this name.

Consequences:

- **Mutable** changes (`MaxNodes`, `Labels`, `Taints`) keep the name — the pool
  is reconciled in place.
- **Immutable** changes flip the hash → a new name → the reconciler creates the
...
- `Name` is a **permanent identifier**: renaming a tier replaces all its pools.
  Uniqueness within a profile makes intra-cluster name collisions structurally
  impossible, so the 24-bit hash only guards change detection.
